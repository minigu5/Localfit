...
def _real_row_to_sample(row: dict) -> tuple[tuple[list[float], float] | None, str | None]:
    ...
    if row.get("benchmark_version") not in (None, 1, 2, 3, 4):
    ...
    if row.get("benchmark_version") == 4:
        sample_count = _bounded_number(row.get("sample_count", 1), 1.0, 10.0)
        sample_min = _bounded_number(
            row.get("tokens_per_sec_min", tokens_per_sec), 0.0, 10_000.0
        )
        sample_max = _bounded_number(
...
def main() -> None:
    ...
    input_sources = [] if args.offline else [
        "firebase_legacy" if "firebaseio.com" in args.telemetry_url else "self_hosted"
    ]
    ...
    print(f"Wrote {args.output} ({len(candidates)} candidates, {len(X)} training rows)")
